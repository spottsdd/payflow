...
for svc in "${SERVICES[@]}"; do
  echo "  installing $svc..."
  (cd "$LANG_DIR/$svc" && python3 -m venv .venv && .venv/bin/pip install --quiet -r requirements.txt)
done

export DATABASE_URL=postgresql://payflow:payflow@localhost:5432/payflow
...
start_service() {
  ...
  local port=$2
  cd "$LANG_DIR/$svc"
  PORT=$port nohup .venv/bin/uvicorn app.main:app --host 0.0.0.0 --port "$port" >"$LOG_DIR/$svc.log" 2>&1 &
  echo $! >> "$PID_FILE"
