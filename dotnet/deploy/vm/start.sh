...
echo "Starting infra (postgres, kafka, gateway-stub)..."
docker compose -f "$INFRA_COMPOSE" up -d --build postgres kafka gateway-stub

echo "Waiting for infra to be healthy..."
for i in $(seq 1 60); do
  healthy=$(docker compose -f "$INFRA_COMPOSE" ps postgres kafka gateway-stub | grep -c "(healthy)" || true)
...
declare -A ASSEMBLIES=(
  ...
  [payment-orchestrator]=PaymentOrchestrator
...
echo "Publishing .NET services..."
for svc in "${SERVICES[@]}"; do
  echo "  publishing $svc..."
  (cd "$LANG_DIR/$svc" && dotnet publish -c Release -o bin/publish --nologo -v quiet)
done

export DATABASE_URL="Host=localhost;Database=payflow;Username=payflow;Password=payflow"
...
start_service() {
  local svc=$1
  local port=$2
  local asm=${ASSEMBLIES[$svc]}
  cd "$LANG_DIR/$svc/bin/publish"
  ASPNETCORE_URLS="http://+:$port" nohup dotnet "$asm.dll" >"$LOG_DIR/$svc.log" 2>&1 &
  echo $! >> "$PID_FILE"
